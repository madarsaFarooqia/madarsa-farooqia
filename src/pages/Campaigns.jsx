...
const CATEGORIES = ['All', 'Infrastructure', 'Scholarships', 'Teacher Salaries', 'Operations', 'Ramadan', 'General'];
      ...
      <div className="pt-28 pb-16 px-4 bg-primary ghost-geometry relative overflow-hidden">
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <span className="inline-flex items-center gap-2 text-xs font-jakarta font-semibold uppercase tracking-[0.2em] mb-4 text-primary-foreground/60">
            <span className="w-6 h-px bg-accent opacity-60"></span>Fundraising<span className="w-6 h-px bg-accent opacity-60"></span>
          </span>
          <h1 className="font-baskerville font-bold text-5xl md:text-6xl text-white mb-4">Our Campaigns</h1>
          <p className="font-jakarta text-primary-foreground/70 text-lg max-w-2xl mx-auto leading-relaxed">
            Every contribution fuels the light of education. See our active campaigns and track where your support goes.
          </p>
        </div>
      </div>
      ...
      <section className="py-16 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: TrendingUp, title: 'Infrastructure', desc: 'Building classrooms, libraries, and facilities for a growing student body.', color: 'bg-primary/10 text-primary' },
              { icon: Heart, title: 'Student Support', desc: 'Food, accommodation, scholarships, and basic needs for underprivileged students.', color: 'bg-accent/10 text-accent' },
              { icon: Users2, title: 'Operations', desc: 'Teacher salaries, utilities, maintenance, and daily running costs of the madrasa.', color: 'bg-primary/5 text-primary' },
          ...
          <div className="mt-14 grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-muted/50 rounded-2xl p-8 text-center">
              ...
              <p className="font-jakarta text-sm text-muted-foreground mt-1">Total Funds Raised</p>
            </div>
            <div className="bg-muted/50 rounded-2xl p-8 text-center">
              <p className="font-baskerville font-bold text-4xl text-primary">{campaigns.length}</p>
              <p className="font-jakarta text-sm text-muted-foreground mt-1">Active Campaigns</p>
            </div>
            <div className="bg-muted/50 rounded-2xl p-8 text-center">
              <p className="font-baskerville font-bold text-4xl text-primary">{totalDonors}</p>
              <p className="font-jakarta text-sm text-muted-foreground mt-1">Generous Donors</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="text-center py-24">
              <p className="font-baskerville text-3xl text-primary mb-4">No Campaigns Found</p>
              <p className="font-jakarta text-muted-foreground">Campaigns will appear here once created by the admin.</p>
            </div>
          ) : (
