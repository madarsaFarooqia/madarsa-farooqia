    ...
    <section className="bg-foreground text-background py-0">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex overflow-x-auto scrollbar-hide">
          {institutions.map(({ id, label, sublabel, icon: Icon, href, description }) => {
            ...
            return (
              ...
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-colors ${
                  isActive ? 'bg-accent text-accent-foreground' : 'bg-white/10 text-white/60 group-hover:bg-white/20'
                }`}>
                  <Icon className="w-4 h-4" />
                </div>
                <div className="text-left min-w-0">
                  <div className={`text-sm font-semibold truncate ${isActive ? 'text-white' : 'text-white/70 group-hover:text-white'}`}>
                    {label}
                  </div>
