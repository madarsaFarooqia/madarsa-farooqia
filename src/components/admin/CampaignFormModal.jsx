    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-border">
          <h2 className="font-playfair font-bold text-xl text-foreground">{campaign ? 'Edit Campaign' : 'New Campaign'}</h2>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <Input value={form.title} onChange={e => set('title', e.target.value)} placeholder="e.g. New Classroom Construction" />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Input type="number" value={form.goal_amount} onChange={e => set('goal_amount', e.target.value)} placeholder="50000" />
            </div>
            <div>
              <Label className="mb-1.5 block text-sm font-medium">Collected Amount</Label>
              <Input type="number" value={form.collected_amount} onChange={e => set('collected_amount', Number(e.target.value))} placeholder="0" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={form.priority} onChange={e => set('priority', e.target.value)} className="w-full border border-input bg-card rounded-md px-3 py-2 text-sm">
